...
from mappolicy.helper.graphics import PointCloud, Quaternion
...
class RLBenchObservationConfig(object):
    ...
    def multi_view_config(camera_names: List[str], image_size: Tuple[int, int] | int):
        ...
        for camera_name in camera_names:
            if camera_name == "wrist":
                obs_config.wrist_camera.set_all(True)
                obs_config.wrist_camera.segmentation_mask = True 
                obs_config.wrist_camera.image_size = image_size
            elif camera_name == "front":
                obs_config.front_camera.set_all(True)
                obs_config.wrist_camera.segmentation_mask = True 
...
class RLBenchEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        # 获取机器人手臂和夹持器的所有零件对象
        arm_objects = self.env.rlbench_task_env._robot.arm.get_objects_in_tree(exclude_base=False)
        gripper_objects = self.env.rlbench_task_env._robot.gripper.get_objects_in_tree(exclude_base=False)
        self.robot_handles = arm_objects + gripper_objects
        # 转换为集合提高查询速度
        self.robot_handle_ids = set([obj.get_handle() for obj in self.robot_handles])
    ...
    def get_point_cloud_single_view(self, camera_name: str, remove_robot: bool = False):
        obs: Observation = self.get_rlbench_env_obs()
        if camera_name == "wrist":
            point_cloud = obs.wrist_point_cloud
            image = obs.wrist_rgb
            mask = obs.wrist_mask  # 获取掩码
        elif camera_name == "front":
        ...
        if remove_robot:
            # 找到不在机器人 ID 列表中的点索引
            # np.isin 会返回一个布尔阵列，invert=True 表示取反（即保留非机器人的点）
            is_not_robot = np.isin(mask, list(self.robot_handle_ids), invert=True)
    ...
    def get_point_cloud(self, remove_robot: bool = False):
        point_clouds = []
        for camera_name in self.point_cloud_camera_names:
            # 传递 remove_robot 参数
            point_cloud = self.get_point_cloud_single_view(camera_name, remove_robot=remove_robot)
            point_clouds.append(point_cloud)
        
        if len(point_clouds) == 0:
            return np.zeros((self.num_points, 6))

        point_cloud = np.concatenate(point_clouds, axis=0)

        # crop point cloud (裁剪代码保持不变)
        ...
        # 安全检查：如果过滤后点太少，无法进行 FPS 采样
        if point_cloud.shape[0] < self.num_points:
            # 随机重复采样补齐点数
            idx = np.random.choice(point_cloud.shape[0], self.num_points, replace=True)
    ...
    def get_obs(self):
        ...
        point_cloud = self.get_point_cloud(remove_robot=False)
        point_cloud_no_robot = self.get_point_cloud(remove_robot=True)
...
if __name__ == "__main__":
    ...
    evaluator = RLBenchEvaluator(
        ...
        camera_name="front",
        point_cloud_camera_names=["front", "left_shoulder", "right_shoulder"],
        use_point_crop=True,
        num_points=4096, # 验证时多存一点点，看得更清楚
        headless=True,
    )

    print("Resetting env and capturing frames for verification...")
    evaluator.env.reset()
    
    # 获取观测字典
    obs = evaluator.env.get_obs()
    pc_with = obs["point_cloud"]
    pc_without = obs["point_cloud_no_robot"]
    img_rgb = obs["image"] 

    # --- 保存图像 (OpenCV) ---
    # RLBench 输出的是 RGB，OpenCV 需要 BGR
    img_bgr = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2BGR)
    cv2.imwrite("verification_img.png", img_bgr)
    print("Saved image to: verification_img.png")

    # --- 保存点云 (Open3D) ---
    def save_to_ply(pc_array, filename):
        # 提取坐标 (N, 3)
        xyz = pc_array[:, :3]
        # 提取颜色 (N, 3)，并确保是 float64/float32 且范围 [0, 1]
        rgb = pc_array[:, 3:6].astype(np.float64)
        if rgb.max() > 1.0:
            rgb /= 255.0
    ...
    save_to_ply(pc_with, "pc_with_robot.ply")
    save_to_ply(pc_without, "pc_no_robot.ply")

    print("\nVerification files generated. You can now download and view them:")
    print("1. Open 'verification_img.png' to see the RGB view.")
    print("2. Use Open3D, MeshLab, or CloudCompare to view the '.ply' files.")