...
class Wall_Desk:
    def __init__(self, sizes, positions, rotations):
        ...
        Edges.append(StructureEdge(0, 1, "Planar-Contact ", {"type": 0, "idx": 2}, {"type": 0, "idx": 3}, [0, 0, 0])) # Top

        self.Nodes = Nodes
        self.Edges = Edges


class StructureMap_ReachWall(StructureGraph):
    def __init__(self, sizes, positions, rotations, clip_model, preprocess=False):
        """        
        :param sizes: [B, 6]
        :param positions: [B, 6]  --  12
        :param rotations: [B, 2*6]  --  24
        Total: [B, 24], Node: 2
        """
        if preprocess:
        ...
        Objects = []
        Objects.append(Wall_Desk(sizes[:, 0:6], positions[:, 0:6], rotations[:, 0:6*2]))
