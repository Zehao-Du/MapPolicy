...
def point_cloud_extracter(
    ...
    for camera_name in point_cloud_camera_names:
        pc = get_point_cloud_single_view(data, camera_name)
        point_clouds.append(pc)
    point_cloud = np.concatenate(point_clouds, axis=0)

    # Crop
    xmin, ymin, zmin, xmax, ymax, zmax = RLBenchEnv.POINT_CLOUD_BOUNDS["default"]
    mask = np.all(point_cloud[:, :3] > [xmin, ymin, zmin], axis=1) & \
           np.all(point_cloud[:, :3] < [xmax, ymax, zmax], axis=1)
    point_cloud = point_cloud[mask]

    # FPS Sampling (增加安全性检查)
...
def main(args):
    ...
    # Create a temporary RLBenchEnv instance to get robot IDs
    ...
    del temp_env  # Release the temporary environment
    ...
    for i in iterable:
        ...
        for j in range(1, len(demo)):
            ...
            point_cloud_arrays_sub.append(
                point_cloud_extracter(demo[j - 1], camera_names, args.num_points, robot_ids=robot_ids, remove_robot=False)
            )
            point_cloud_no_robot_arrays_sub.append(
                point_cloud_extracter(demo[j - 1], camera_names, args.num_points, robot_ids=robot_ids, remove_robot=True)
            )
