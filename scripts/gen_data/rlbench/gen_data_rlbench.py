...
from mappolicy.dataset.rlbench_dataset import RLBenchDataset
from mappolicy.envs.rlbench_env import RLBenchEnv, RLBenchObservationConfig
from mappolicy.helper.common import (
    save_depth_image,
...
from mappolicy.helper.graphics import EEpose, PointCloud, Quaternion
...
def robot_state_extracter(data):
...
def image_extracter(data):
    return data.front_rgb


def depth_extracter(data):
    return data.front_depth


def point_cloud_extracter(
    data,
    point_cloud_camera_names: list,
    num_points: int,
    robot_ids: set = None,
    remove_robot: bool = False,
):
    def get_point_cloud_single_view(data, camera_name: str):
        point_cloud = getattr(data, f"{camera_name}_point_cloud")
        image = getattr(data, f"{camera_name}_rgb")
        mask = getattr(data, f"{camera_name}_mask") if remove_robot else None
        ...
        if remove_robot and robot_ids is not None:
            mask = mask.reshape(-1)
            is_not_robot = np.isin(mask, list(robot_ids), invert=True)
            point_cloud = point_cloud[is_not_robot]
            image = image[is_not_robot]

        return np.concatenate((point_cloud, image), axis=-1)

    point_clouds = []
    for camera_name in point_cloud_camera_names:
        point_clouds.append(get_point_cloud_single_view(data, camera_name))
    point_cloud = np.concatenate(point_clouds, axis=0)

    xmin, ymin, zmin, xmax, ymax, zmax = RLBenchEnv.POINT_CLOUD_BOUNDS["default"]
    mask = np.all(point_cloud[:, :3] > [xmin, ymin, zmin], axis=1) & np.all(
        point_cloud[:, :3] < [xmax, ymax, zmax], axis=1
    )
    ...
    return PointCloud.point_cloud_sampling(point_cloud, num_points, "fps")


def _build_action_points_without_keyframe(demo_len: int, interval: int):
    action_points = list(range(0, demo_len, interval))
    if len(action_points) == 0:
        action_points = [0]
    if action_points[-1] != demo_len - 1:
        action_points.append(demo_len - 1)
    return sorted(set(action_points))
...
def main(args):
    ...
    Logger.log_info("Keyframe extraction: disabled")
    ...
    from mappolicy.envs.rlbench_env import RLBenchActionMode
    ...
    temp_env = RLBenchEnv(
        ...
        point_cloud_camera_names=args.point_cloud_camera_names,
        use_point_crop=True,
        num_points=args.num_points,
        headless=True,
    )
    robot_ids = temp_env.robot_handle_ids
    del temp_env
    ...
    for i in iterable:
        ...
        action_points = _build_action_points_without_keyframe(
            demo_len=len(demo), interval=args.interval
        )

        if not args.quiet:
            Logger.log_info(
                f'extracted {colored(len(action_points), "red")} action points from '
                f'{colored(len(demo), "red")} steps (without keypoints)'
            )

        demo = [demo[idx] for idx in action_points]
        for j in range(1, len(demo)):
            ...
            depth_arrays_sub.append(depth_extracter(demo[j - 1]))
            point_cloud_arrays_sub.append(
                point_cloud_extracter(
                    demo[j - 1],
                    camera_names,
                    args.num_points,
                    robot_ids=robot_ids,
            ...
            point_cloud_no_robot_arrays_sub.append(
                point_cloud_extracter(
                    demo[j - 1],
                    camera_names,
                    args.num_points,
        ...
        save_point_cloud_ply(
            ...
            os.path.join(point_cloud_dir, f"episode_{i}_no_robot.ply"),
            quiet=args.quiet,
        )
    ...
    point_cloud_no_robot_arrays = np.stack(point_cloud_no_robot_arrays, axis=0)
    ...
    depth_chunk_size = (100, *depth_arrays.shape[1:])
    point_cloud_chunk_size = (100, point_cloud_arrays.shape[1], point_cloud_arrays.shape[2])
    point_cloud_no_robot_chunk_size = (
        100,
        point_cloud_no_robot_arrays.shape[1],
        point_cloud_no_robot_arrays.shape[2],
    )
    ...
    zarr_data.create_dataset(
        ...
        chunks=point_cloud_no_robot_chunk_size,
        dtype="float32",
        compressor=compressor,
    )
    zarr_data.create_dataset(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rlbench-data-root", type=str, default="/data2/zehao/MapPolicy/Data/RLBench_224")
    ...
    parser.add_argument(
        "--save-dir",
        type=str,
        default=str(pathlib.Path(__file__).resolve().parent.parent.parent.parent / "data" / "rlbench"),
    )
